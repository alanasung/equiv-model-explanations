# Design Document: Explanations That Survive Functionally Equivalent Models

## 1. Purpose

This document is the method contract for **Explanations That Survive Functionally Equivalent Models**. It specifies what is
...
One-liner: Check whether component-level explanations remain stable across models trained to the same function.

Hypothesis: Check whether component-level explanations remain stable across models trained to the same function.
...
|---|---|---|
| Config schema | `equivxai.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `equivxai.configs.loader` | Hydra compose API |
| Device | `equivxai.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `equivxai.models.registry` | Pinned revisions + hardware fit |
| Hooks | `equivxai.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `equivxai.models.generation` | Chat template path recording |
| Data | `equivxai.data` | Synthetic items, splits, manifests |
| Evaluation | `equivxai.evaluation` | Metrics, failure harness, protocols |
| Ablation | `equivxai.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `equivxai.reporting` | Aggregate → MD/TeX/figures |
| Cache | `equivxai.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/equiv-model-explanations
...
`src/equivxai/equivxai/` are preserved across regeneration.
