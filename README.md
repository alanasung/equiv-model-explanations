<p align="center">
  ...
  <p align="center">SPAR Fall 2026 · <a href="https://github.com/alanasung/equiv-model-explanations">Explanations That Survive Functionally Equivalent Models</a></p>
</p>
...
**Explanations That Survive Functionally Equivalent Models**. The shared engineering spine (config, caching, hooks, metrics,
...
| Stage | Module | Output |
|---|---|---|
| Compose config | `configs/` + `equivxai.configs` | resolved `config.yaml` |
| Build data | `equivxai.data` | splits + manifest |
| Load model | `equivxai.models` | `LoadedModel` + resolved commit |
| Run stages | `scripts/run_experiment.py` | per-stage JSON |
| Aggregate | `equivxai.reporting` | `results.json` + tables + figures |
...
├── src/equivxai/       # installable library (print-free)
...
- SPAR Fall 2026 project posting: https://github.com/alanasung/equiv-model-explanations
- Complexity bar: Critical Data PRIMED-AI / RecursiveJEPA engineering standard

## Citation

```bibtex
@misc{equivxai_spar_f26,
  title        = {Explanations That Survive Functionally Equivalent Models},
  author       = {Alana Sung},
  year         = {2026},
  howpublished = {SPAR Fall 2026},
  note         = {Check whether component-level explanations remain stable across models trained to the same function.},
  url          = {https://github.com/alanasung/equiv-model-explanations}
}
```
...
Ablations live under `src/equivxai/ablation/` and return structured dicts. Presets
